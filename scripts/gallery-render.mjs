#!/usr/bin/env node
/**
 * Render one promo video per recorded gallery take.
 *
...
const TAKES = join(PROMO, "public", "gallery");
const OUT = join(PROMO, "out", "gallery");
...
for (const slug of slugs) {
  ...
  const props = {
    ...
    liveAt: (meta.liveAt || 0) / 1000,
    endAt: (meta.endAt || 0) / 1000,
    box: meta.box || null,
  };
...
console.log(`\n${done}/${slugs.length} videos in promo/out/gallery/`);
