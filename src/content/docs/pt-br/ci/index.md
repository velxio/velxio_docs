---
title: Velxio CI
description: Execute seu firmware em uma placa simulada a partir de um terminal ou de um job de CI, e falhe o build quando o firmware se comportar mal.
sidebar:
  order: 1
  badge:
    text: Paid
...
comando sai com código diferente de zero quando algo que você pediu não aconteceu.

```bash
curl -fsSL https://velxio.dev/ci/install.sh | sh
velxio-cli login                           # aprove no navegador, uma vez
cd firmware/blink
...
- **Testar o que você não pode manter em uma mesa.** Toda placa que o Velxio simula está
  disponível para todo job, em paralelo, sem laboratório e sem gravação.
- **Manter o toolchain que você já tem.** Compile com arduino-cli, ESP-IDF,
  PlatformIO ou cargo em uma etapa anterior; o Velxio só executa o que saiu.
...
em toda placa, seja o emulador mais rápido ou mais lento que o tempo real.
...
Dois arquivos no diretório para o qual você aponta a CLI:

- `velxio.toml` — a placa e o firmware. Um `wokwi.toml` do Wokwi também funciona.
- `diagram.json` — o circuito. O formato do Wokwi, então um diagrama existente roda
  sem alterações.

Adicione `scenario.yaml` quando uma verificação serial não for suficiente: ele pode esperar por texto,
enviar texto, esperar no relógio simulado, verificar um pino e definir um controle em uma
peça. Veja [Cenários](/docs/pt-br/ci/scenarios/).
