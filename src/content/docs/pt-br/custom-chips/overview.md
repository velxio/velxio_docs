...
SPI ou UART, expor atributos ao painel de propriedades e até desenhar em um
framebuffer.

Nesta seção:

- **Getting started** — crie seu primeiro custom chip a partir do editor.
- **Programmable sensors** — controles deslizantes ao vivo que acionam seu chip enquanto a
  simulação é executada (`controls` em chip.json).
- **My Chips** — salve um chip uma vez e reutilize-o em qualquer projeto (Pro).
