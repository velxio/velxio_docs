...
2. Klicken Sie auf **Connect & flash**. Der Browser fragt, welches USB-Gerät er freigeben soll; wählen Sie Ihr Board.
...
Ein RP2040 oder RP2350 wird von seinem Bootloader programmiert, einer separaten USB-Persona, die der Chip nur im **BOOTSEL**-Modus zeigt. Zwei Wege dorthin:
...
- **Über den Dialog**: Der Flash-Dialog für diese Boards hat einen **Reboot into bootloader over USB**-Button. Er funktioniert, wenn das Board ein Sketch ausführt, das Velxio erstellt hat (der Arduino-Kern startet bei einem 1200-Baud-Open neu) oder MicroPython (die REPL führt `machine.bootloader()` aus). Der Browser fragt nach dem seriellen Port des Boards, das Board meldet sich ab und kommt als Bootloader zurück. Klicken Sie dann auf **Connect & flash** und wählen Sie das Gerät `RP2 Boot` / `RP2350 Boot`.
...
dann `sudo udevadm control --reload-rules && sudo udevadm trigger` und stecken Sie das Board neu ein. Der für den Neustart verwendete serielle Port erfordert außerdem die übliche `dialout`-Gruppenmitgliedschaft.

### Jeder Browser: die .uf2 herunterladen

Der Flash-Dialog für ein Pico-Familien-Board bietet immer **Download .uf2** an (bei Firefox und Safari, wo der Browser nicht flashen kann, ist das der gesamte Dialog). Speichern Sie die Datei, versetzen Sie das Board in den BOOTSEL-Modus und legen Sie die Datei auf dem Laufwerk `RPI-RP2` / `RP2350` ab: Das Board startet in dem Moment, in dem das Kopieren endet, in Ihr Sketch neu.

### MicroPython-Projekte auf einem Pico

Der Dialog lädt die `.py`-Dateien des Projekts über die REPL hoch und startet in `main.py` neu. MicroPython selbst muss bereits auf dem Board sein: Es ist eine `.uf2`, die Sie einmal auf dem BOOTSEL-Laufwerk ablegen (Pimoroni-Boards werden damit ausgeliefert; Downloads unter
[pimoroni-pico-rp2350](https://github.com/pimoroni/pimoroni-pico-rp2350/releases)
und [micropython.org](https://micropython.org/download/)).
...
- **"The board in BOOTSEL is an RP2040 but this project is built for RP2350"**: Pimoroni verkaufte den Stellar und Galactic Unicorn bis Januar 2025 mit einem Pico W (RP2040) und seitdem mit einem Pico 2 W (RP2350). Überprüfen Sie das Etikett an Ihrem Gerät und wählen Sie das passende Board im Editor.
