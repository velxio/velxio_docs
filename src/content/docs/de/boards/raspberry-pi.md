...
meisten tun es: Die Tabellen unten wurden gegen das Live-Produkt gemessen,
Teil für Teil, am 2026-09-19.
...
Jeder kann einen Pi auf der Arbeitsfläche platzieren und eine Schaltung
darum herum verdrahten. Ihn **auszuführen** erfordert einen bezahlten Plan
oder eine der **drei kostenlosen Testsitzungen von 15 Minuten**, die jedes
angemeldete Konto für die Pi-Familie erhält (siehe
[Pläne](/docs/de/getting-started/plans/)). Ein Projekt, das nichts enthält, was
ein Pi ausführen kann, wie etwa ein Arduino-`.ino`-Sketch auf einem
Pi-Board, wird darauf hingewiesen, bevor eine Testsitzung dafür verbraucht
wird.
...
`subprocess`, keine rohen Sockets. Ein Skript, das eines davon anfordert,
wird stattdessen an die Linux-Engine geschickt; der **Engine-Chip** in der
...
- **Der Header-UART ist ein echter serieller Port.** `/dev/serial0` (auch
  `/dev/ttyAMA0` und `/dev/ttyS0`) ist ein echtes tty, das vom unveränderten
  pyserial angesteuert wird: `serial.tools.list_ports`, `select()` auf dem
  Port und `cat /dev/serial0` funktionieren alle, und die Bytes gehen an
  das, was auf der Arbeitsfläche mit GPIO14 und GPIO15 verdrahtet ist.
- **1-Wire ist als der sysfs-Baum vorhanden, den ein Pi hat.** Ein DS18B20 an
  GPIO4 erscheint unter `/sys/bus/w1/devices/28-*/` mit `w1_slave` und
  `temperature`, sodass `cat`, Leser im `w1thermsensor`-Stil und dein eigener
  Code funktionieren (`dtoverlay=w1-gpio,gpiopin=N` in einer `config.txt` des
  Projekts verschiebt den Pin).
- **`libcamera-jpeg`, `rpicam-jpeg`, `rpicam-still`** machen ein Standbild
  vom Kamera-Teil auf der Arbeitsfläche, so wie ein Skript sie mit
  `subprocess` aufruft. In dieser Engine ist das Bild das **Testmuster** des
  Teils: Der Gast läuft auf Velxios Servern, und die Pixel deiner Webcam
  verlassen niemals deinen Browser.
- Es gibt **kein** `/dev/gpiomem`, `/dev/gpiochip0` oder `/sys/class/gpio`.
  GPIO läuft über `RPi.GPIO` und `gpiozero`, die vorhanden sind; `libgpiod`,
  `gpioinfo` und `pigpio` haben nichts, womit sie sprechen könnten.
...
den Rest der Sitzung fest, wenn du die Shell möchtest, zum Beispiel um
Dateien zu inspizieren oder ein Skript von Hand auszuführen. Die Wahl wird
nicht mit dem Projekt gespeichert: Öffne es morgen erneut, und Run kehrt zur
Antwort des Detektors zurück. Alles, was die Instant-Engine ausführen kann,
ist ohne sie schneller.

## Was funktioniert, Teil für Teil

Jede Zeile ist ein Skript, geschrieben so, wie ein Pi-Tutorial es schreibt,
ausgeführt durch das Live-Produkt mit dem auf der Arbeitsfläche verdrahteten
Teil, auf einem Raspberry Pi 4. Die Display-Zeilen werden auf der
Arbeitsfläche selbst überprüft: Das Panel muss leuchten, nicht nur das
...
| GPS-Modul am Header-UART | `pyserial` auf `/dev/serial0` | Ja | Ja |
...
- **Deine Webcam in der Linux-Engine.** `picamera2` in der Instant-Engine
  kann deine Webcam verwenden, weil das Skript in deinem Browser läuft. Der
  Gast läuft auf unseren Servern, daher bekommen seine Kamera-Tools
  stattdessen das Testmuster.
- **Ein E-Paper-Treiber, der sein Bild an die falsche Stelle schickt.** Bei
  einem UC8179-Panel (dem 7,5") ist Befehl `0x10` das vorherige Bild und
  `0x13` das, welches das Glas zeigt. Ein Treiber, der nur `0x10` schreibt,
  bekommt hier eine leere Aktualisierung, genau wie auf dem echten Panel, und
  der serielle Monitor (das Linux-Terminal, in dieser Engine) sagt warum. Der
  BUSY-Pin folgt ebenfalls dem Controller: LOW, während ein UltraChip-Panel
  arbeitet, HIGH bei einem SSD168x.
...
dass es allein ab der `import`-Zeile funktioniert, ohne `requirements.txt`,
so wie Raspberry Pi OS seine Hardware-Bibliotheken im Image hat.
...
| `w1thermsensor` | Vorinstalliert | Über `requirements.txt` (der 1-Wire-Baum ist vorhanden) |
...
### Andere Drittanbieter-Pakete benötigen eine `requirements.txt`
...
Ausführungskonsole mit, was es installiert hat. Wenn ein Skript ein Paket
importiert, das fehlt, bietet die Konsole die hinzuzufügende Zeile an, und
eine Schaltfläche schreibt sie für dich. Welche Engine ein Paket aufnehmen
...
`unihiker`-Modulen anstelle der Pi-Shims. Es ist ein bezahltes Board mit
seinen eigenen drei Testsitzungen; du findest es im Auswahlmenü neben der
Pi-Familie.
