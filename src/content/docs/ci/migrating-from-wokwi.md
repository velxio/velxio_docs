...
`elf`. Mint the token at
[velxio.dev/account/ci](https://velxio.dev/account/ci) and store it as a
...
| `board-velxio-<kind>`                            | any of the above, written the Velxio way      |
...
phase when it is planned. That includes the remaining ESP32 devkits
(`board-esp32-devkit-c-v4`, `board-esp32-cam`, `board-wemos-lolin32-lite`,
the XIAO ESP32 boards, `board-arduino-nano-esp32`, the C3 SuperMini, the P4
boards), `board-pi-pico-2` and `-2w`, the STM32 boards, the Nucleos, the
ESP32-S2/H2 boards and the display kits. A nearby Velxio board is suggested
only when it runs today, and it is never substituted for you. Nothing is
billed for a refusal.
