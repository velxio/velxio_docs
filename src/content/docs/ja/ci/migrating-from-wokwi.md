...
description: wokwi-cli のジョブを Velxio CI に移すと何が変わるのか - uses 行、シークレット名 - そして変わらないもの。
sidebar:
  order: 7
---
...
独自のパーサーがこれらのフォーマットを読み取ります。実際の移行は 2 行です。
...
`elf`。シークレットは `velxio-cli login --ci --name "<repo>"` で取得し（ブラウザで承認され、トークンが一度だけ表示されます）、
リポジトリシークレットとして保存します。完全なリストは [GitHub Actions](/docs/ja/ci/github-action/) にあります。
...
`--timeout` はどちらもシミュレートされたミリ秒です。

`WOKWI_CLI_TOKEN` は決して読み取られません。`VELXIO_CLI_TOKEN`（または
`VELXIO_CI_TOKEN`）を設定するか、一度 `velxio-cli login` を実行してください。
...
サポートされていないキーは名前で報告されます。これらは警告であり、
黙って省略されることはありません - ただし `[[chip]]` は例外で、テスト対象のチップを欠いた回路で
パスしてしまうことがないように実行を停止します。現在の Velxio CI には GDB サーバー、RFC2217 ポート、
VCD エクスポート、ネットワーク転送はありません。

Velxio が追加するキー - `board`、`diagram`、`project`、`scenario`、
`flasher_args` - は `[velxio]` の下にあります。
...
| Wokwi `diagram.json` タイプ | Velxio の種類 |
| ------------------------- | ----------- |
...
| `board-velxio-<kind>` | CI が実行する任意のボードを Velxio の書き方で記述 |

Velxio は CI で 36 のボードを実行し、そのほとんどは Wokwi に対応するタイプがないボードです -
RP2350 ファミリー、XIAO ARM ボード、M5Stack および Seeed のキット。これらは
`board-velxio-<kind>` として記述します。完全なリストは
[ボードテーブル](/docs/ja/ci/velxio-toml/) にあります。

その他のすべての Wokwi ボードは実行開始前に失敗し、
`board_not_supported_in_ci` または `unknown_board_type`、タイプ名、および計画されたフェーズが示されます。
これには `board-pi-pico-2` と `-2w`、STM32 ボード、Nucleo、ESP32-S2/H2/C61 ボード、ESP32-P4 プレビュー
開発キット、ディスプレイキットが含まれます。近い Velxio ボードは現在動作する場合にのみ提案され、
自動的に置き換えられることはありません。拒否に対して課金されることはありません。

`velxio-cli boards` は各ボードのステータスとともにライブリストを出力します。

## シナリオ

Wokwi のシナリオ YAML はそのまま実行されます: `delay`、`wait-serial`、
`write-serial`、`expect-pin`、`set-control`、`take-screenshot`、同じフィールド名
（`part-id`、`save-to`、`compare-with`、`value`）で。すべてのタイミングは
Wokwi と同様にシミュレート時間です。2 つの違いがあります:

- **タッチステップ**（`touch-press`、`touch-move`、`touch-release`）は
  実装されていません。CLI は lint 時にこれらを拒否します。
- **`compare-with` はキャプチャされますが、まだ比較されません**: PNG と
  警告が得られ、実行はそれによって失敗しません。

詳細は [シナリオ](/docs/ja/ci/scenarios/) にあります。

## ファームウェア

CLI はツールチェーンが生成したものをボードのエンジンが読み込むものに変換します:

- Arduino ESP32 の「Export compiled binary」フォルダは動作します: `<sketch>.ino.bin`
  は `<sketch>.ino.bootloader.bin` および
  `<sketch>.ino.partitions.bin`（存在する場合は `boot_app0.bin` も）とマージされます。
- PlatformIO の `firmware.bin` + `bootloader.bin` + `partitions.bin` も
  同じようにマージされます。ESP-IDF プロジェクトは代わりに `flasher_args` を
  `build/flasher_args.json` に向けることができます。
- 兄弟ファイルのない単独の ESP32 `app.bin` は拒否され、
  `esptool.py merge_bin` のヒントが表示されます。
- Pico の `.uf2` と `.elf` はフラッシュイメージにフラット化されます。AVR の `.elf`
  は Intel HEX になります。
- ブートローダーのチップ ID はボードと一致する必要があります: `esp32-s3` ボード上の
  ESP32-C3 イメージは `firmware_format_mismatch`、終了コード 2 になります。
...
分はシミュレート時間で、秒単位に切り上げられ、暦月（UTC）ごとです:
Maker では月 200 分、Pro では 2,000 分。開始前に拒否された実行は無料で、
エンジンの停止や実時間の上限は経過したシミュレート秒数のみのコストです。完全な表は
[終了コード](/docs/ja/ci/exit-codes/) を参照してください。
