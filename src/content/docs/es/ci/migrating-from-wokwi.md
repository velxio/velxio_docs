...
`diagram.json` y el YAML de escenarios de Wokwi. No interviene nada del código
de Wokwi; nuestros propios analizadores leen esos formatos. En la práctica, la
...
Las entradas de la action conservan sus nombres a propósito: `path`, `timeout`,
`expect_text`, `fail_text`, `scenario`, `serial_log_file`, `diagram_file`,
`elf`. Obtén el secreto con `velxio-cli login --ci --name "<repo>"` (se aprueba
en el navegador e imprime el token una sola vez) y guárdalo como secreto del
repositorio. La lista completa está en [GitHub Actions](/docs/es/ci/github-action/).
...
no omisiones silenciosas, excepto `[[chip]]`, que detiene la ejecución para que
nunca obtengas un aprobado de un circuito al que le falta el chip bajo prueba.
Hoy no hay servidor GDB, ni puerto RFC2217, ni exportación VCD, ni reenvío de
red en Velxio CI.

Las claves que añade Velxio (`board`, `diagram`, `project`, `scenario`,
`flasher_args`) viven bajo `[velxio]`. Consulta
[velxio.toml](/docs/es/ci/velxio-toml/).

## Placas

Los tipos de componente de Wokwi se corresponden con los kinds de Velxio. Hoy
funcionan estos:

| Tipo de `diagram.json` de Wokwi | Kind de Velxio |
| ------------------------- | ----------- |
...
`board_not_supported_in_ci` o `unknown_board_type`, el tipo nombrado y la fase
en la que está prevista. Eso incluye `board-pi-pico-2` y `-2w`, las placas
STM32, las Nucleo, las placas ESP32-S2/H2/C61, el devkit de vista previa
ESP32-P4 y los kits con pantalla. Solo se sugiere una placa Velxio cercana
cuando funciona hoy, y nunca se sustituye por ti. Un rechazo no se factura.
...
nombres de campo (`part-id`, `save-to`, `compare-with`, `value`). Toda la
temporización es tiempo simulado, como en Wokwi. Dos diferencias:

- Los **pasos táctiles** (`touch-press`, `touch-move`, `touch-release`) no
  están implementados; la CLI los rechaza en el momento del lint.
...
La CLI convierte lo que produjo tu toolchain en lo que carga el motor de la
placa:
...
- `firmware.bin` + `bootloader.bin` + `partitions.bin` de PlatformIO se
...
- El chip id del bootloader debe coincidir con la placa: una imagen de ESP32-C3
  en una placa `esp32-s3` es `firmware_format_mismatch`, salida 2.

MicroPython no es compatible: las ejecuciones de CI usan firmware compilado, y
...
tiempo real solo cuestan los segundos simulados que habían transcurrido.
Consulta [Códigos de salida](/docs/es/ci/exit-codes/) para la tabla completa.
...
Si `lint` no da problemas, la ejecución llegará a una placa.
