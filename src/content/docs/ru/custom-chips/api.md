...
Все, что может делать чип, объявлено в **`velxio-chip.h`**. Хост
вызывает вашу экспортируемую функцию `chip_setup()` один раз на экземпляр;
в ней вы регистрируете контакты и периферию и подключаете обратные вызовы.
Все дальнейшее выполнение происходит в этих обратных вызовах.
...
уровня без дребезга между регистрацией и первой записью.
...
double  vx_attr_read(vx_attr a);   // перечитывайте в колбэках — слайдеры меняют его в реальном времени
...
`on_rx_byte` срабатывает на каждый принятый байт; `on_tx_done` — когда ваш буфер
отправлен.
...
ПЗУ позволяет чипу нести внешние данные (знакогенераторы, микрокод), внедряемые
хостом до вызова `chip_setup()`.

## Внешний вид чипа

Корпус рисуется из `chip.json`: список контактов размещает площадки и их
подписи, а необязательный `display: { width, height }` резервирует область
фреймбуфера. Чип также может нести **изображение** — PNG, JPEG или SVG,
добавленное в его файловый раздел как `chip.png` / `chip.jpg` / `chip.svg` — которое
покрывает корпус, не перемещая ни одного контакта. См.
[Придание чипу внешнего вида](/docs/ru/custom-chips/getting-started/#giving-the-chip-a-face).
...
`pins` определяет порядок физического корпуса; имена должны совпадать с тем, что
