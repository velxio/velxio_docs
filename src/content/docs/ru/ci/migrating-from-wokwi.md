...
`diagram.json` и YAML-сценарии Wokwi. Код Wokwi не задействован;
наши собственные парсеры читают эти форматы. На практике миграция — это две строки.
...
Имена входных параметров action намеренно сохранены: `path`, `timeout`,
`expect_text`, `fail_text`, `scenario`, `serial_log_file`, `diagram_file`,
`elf`. Получите секрет с помощью `velxio-cli login --ci --name "<repo>"` (он
подтверждается в браузере и один раз выводит токен) и сохраните его как
секрет репозитория. Полный список в [GitHub Actions](/docs/ru/ci/github-action/).
...
`--timeout` в обоих случаях — это смоделированные миллисекунды.
...
Читается как есть — переименовывать в `velxio.toml` не нужно:
...
молчаливое игнорирование — кроме `[[chip]]`, который останавливает запуск, чтобы вы
никогда не получили успех от схемы без тестируемого чипа. В Velxio CI сегодня нет
GDB-сервера, порта RFC2217, экспорта VCD и перенаправления сети.
...
Типы компонентов Wokwi сопоставляются с видами Velxio. Сегодня работают
следующие:
...
Velxio запускает в CI тридцать шесть плат, и большинство из них — платы, для которых
у Wokwi нет типа: семейство RP2350, платы XIAO на ARM, наборы M5Stack и Seeed.
Записывайте их как `board-velxio-<kind>`; полный список в
[таблице плат](/docs/ru/ci/velxio-toml/).

Любая другая плата Wokwi завершается ошибкой до начала запуска, с
`board_not_supported_in_ci` или `unknown_board_type`, с указанием типа и
фазы, на которой он планируется. Это касается `board-pi-pico-2` и `-2w`,
плат STM32, Nucleo, плат ESP32-S2/H2/C61, предварительного devkit
ESP32-P4 и наборов с дисплеем. Близкая плата Velxio предлагается только тогда, когда она
работает сегодня, и она никогда не подставляется за вас. За отказ
...
YAML-сценарии Wokwi выполняются без изменений: `delay`, `wait-serial`,
`write-serial`, `expect-pin`, `set-control`, `take-screenshot`, с теми же
именами полей (`part-id`, `save-to`, `compare-with`, `value`). Всё время —
смоделированное, как в Wokwi. Два отличия:

- **Шаги касания** (`touch-press`, `touch-move`, `touch-release`) не
  реализованы; CLI отклоняет их на этапе lint.
- **`compare-with` захватывается, но пока не сравнивается**: вы получаете PNG и
  предупреждение, и запуск из-за этого не завершается ошибкой.

Подробности в [Сценариях](/docs/ru/ci/scenarios/).

## Прошивка

CLI превращает то, что создала ваша цепочка инструментов, в то, что загружает
движок платы:
...
- `firmware.bin` + `bootloader.bin` + `partitions.bin` из PlatformIO объединяются
  так же. Проекты ESP-IDF могут вместо этого указать `flasher_args` на
...
- Идентификатор чипа загрузчика должен совпадать с платой: образ ESP32-C3 на
  плате `esp32-s3` — это `firmware_format_mismatch`, код выхода 2.
...
Минуты — это смоделированное время, округлённое вверх до целых секунд, за календарный месяц
(UTC): 200 в месяц на Maker, 2 000 на Pro. Запуск, отклонённый до
начала, ничего не стоит, а зависший движок или ограничение по реальному времени стоят только
истёкшие смоделированные секунды. Полную таблицу см. в
[Кодах выхода](/docs/ru/ci/exit-codes/).
