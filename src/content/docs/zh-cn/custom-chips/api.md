...
芯片能做的所有事情都在 **`velxio-chip.h`** 中声明。主机会为每个实例调用你导出的 `chip_setup()` 一次；在那里你注册引脚和外设并挂接回调。所有后续执行都发生在这些回调中。
...
在片选信号有效期间交换缓冲区——MCP3008 示例展示了完整的请求/响应交互过程。
...
ROM 让芯片携带外部数据（字符 ROM、微码），这些数据由主机在 `chip_setup()` 之前注入。

## 芯片的外观

主体由 `chip.json` 绘制：引脚列表放置焊盘及其标签，可选的 `display: { width, height }` 保留一个帧缓冲区域。芯片还可以携带**图像**——PNG、JPEG 或 SVG，添加到其文件部分，命名为 `chip.png` / `chip.jpg` / `chip.svg`——该图像覆盖主体而不移动任何引脚。参见[赋予芯片外观](/docs/zh-cn/custom-chips/getting-started/#giving-the-chip-a-face)。
...
`pins` 定义物理封装顺序；名称必须与 C 源代码注册的相匹配。可选部分：`attributes`（可调值）、`controls`（仿真期间的实时滑块/按钮）、`display`（`{"width", "height"}`，用于帧缓冲芯片）和 `programTargets`（运行用户程序的复古 CPU 芯片）。
