...
`velxio.toml` dice alla CLI cosa eseguire: quale board, quale firmware
compilato, quale circuito e quale scenario. Si trova nella directory
indicata alla CLI. Ogni percorso al suo interno è relativo al file stesso,
e le barre in avanti funzionano su ogni sistema operativo.
...
| `board`        | il tipo di board Velxio (tabella sotto). Opzionale quando il diagram o il `.vlx` nominano già la board; quando entrambi sono presenti devono concordare. |
| `firmware`     | l'immagine compilata: `.hex`, `.bin`, `.uf2`, o un'immagine flash ESP32 unificata.                                                           |
| `flasher_args` | un `build/flasher_args.json` di ESP-IDF invece di `firmware`. I due sono mutuamente esclusivi.                                                |
...
| `project`      | un'esportazione di progetto Velxio `.vlx`. Prevale su `diagram`.                                                                             |
...
da quello che hai scritto. `[[chip]]` (chip personalizzati) è una di quelle:
viene rifiutato oggi, con il file sorgente indicato.

## Board che CI esegue oggi

È il server a decidere, non la CLI. Trentasei tipi funzionano ora — ogni
...
`velxio-cli boards` stampa la lista aggiornata con lo stato di ogni board, i
suoi tipi `diagram.json` e i formati firmware che accetta.

:::caution
Ciò che resta viene eseguito nell'editor ma non ancora in CI: le board STM32
(necessitano della corsia QEMU), le board Raspberry Pi e UNIHIKER, il devkit
di anteprima ESP32-P4, e la famiglia DFRobot, che è ancora dietro il suo
flag di lancio. Ognuna viene rifiutata prima che l'esecuzione inizi, con
`board_not_supported_in_ci` e la fase in cui è pianificata. Nulla viene
fatturato, e nessuna board vicina viene sostituita in silenzio.
:::

Pico W funziona, ma CI non ha rete: WiFi e socket non si connettono mai, e
l'esecuzione porta un avviso `no_network`.

## Come si risolvono i percorsi

- **File di configurazione:** `velxio.toml`, poi `wokwi.toml`, poi esattamente un
  `*.vlx` nella directory. Nessuno di essi è exit 2.
...
I percorsi relativi forniti sulla riga di comando si risolvono rispetto
...
simulare vengono segnalate per id invece che eliminate in silenzio.

## .vlx

Un progetto esportato dall'editor Velxio (`format: "velxio-project"`,
`version: 1`) può essere il circuito invece di un diagram. Metti l'unico
...
| scenario                   | 200 passi, 20 screenshot, 512 byte per testo `wait-serial`                                         |
...
percorso, verifica che i file esistano e rispettino i limiti, verifica che
gli id delle parti siano univoci e che le connessioni nominino parti
esistenti, verifica che la board sia una che CI esegue, verifica che il
formato del firmware corrisponda alla famiglia della board, e verifica che
ogni passo di scenario sia noto, abbia i suoi campi, nomini parti esistenti
e analizzi le sue durate. La maggior parte dei fallimenti `exit 2` è più
economico trovarli qui.
