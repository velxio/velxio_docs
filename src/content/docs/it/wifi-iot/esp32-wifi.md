...
description: Unisciti alla rete integrata Velxio-GUEST e raggiungi Internet reale da un ESP32 simulato.
sidebar:
  order: 2
---
...
componente **WiFi Access Point** alla tela (cerca "WiFi Access Point" nel
selettore componenti) e la radio emulata trasmette **il tuo SSID** invece.
Lo sketch si connette quindi alla rete che nomina effettivamente:
...
più componenti per esercitare un'interfaccia di selezione rete; ciascuno ha
il proprio canale e intensità del segnale, e scansioni ripetute variano di
pochi dB come quelle reali.

Due proprietà meritano attenzione:

- **Internet** — spegnila e la rete diventa isolata: la scheda si associa
  e ottiene un IP tramite DHCP, ma nulla viene instradato all'esterno. Questo
  è lo scenario di provisioning / portale captive, ora testabile nel simulatore.
- **Password** — memorizzata con il componente e mostrata sulla sua scheda,
  ma la rete trasmette comunque autenticazione aperta finché non arriva
  l'emulazione WPA2. Gli sketch che passano una password si connettono comunque.

Anche il firmware caricato ne beneficia: un binario compilato altrove si
connette a qualsiasi rete nomini, purché un punto di accesso trasmetta quel
...
attraverso il gateway IoT. La piccola freccia accanto apre il **pannello WiFi**:
...
- **Download PCAP** — il traffico 802.11 dell'esecuzione come file di cattura
  ...
  nel tuo browser;
- l'abbinamento [gateway di rete locale](/docs/it/wifi-iot/local-gateway/).
...
schede ESP32, ESP32-S3, ESP32-C3, ESP32-C6 ed ESP32-C5 (e le loro varianti
XIAO / Nano / M5Stack). Lo stato di advertising Bluetooth viene anche
segnalato per gli sketch che inizializzano BLE.
